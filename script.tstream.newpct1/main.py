...
WEB_PAGE_BASE = 'https://descargas2020.com'
...
SEARCH_URI_BASE = '%s/index.php?page=buscar&q=%s&categoryIDR=&idioma=&pg=1' % (WEB_PAGE_BASE, '%s')
MOVIE_URI_BASE = '%s/peliculas/pg/1' % WEB_PAGE_BASE
PREMIERE_URI_BASE = '%s/estrenos-de-cine/pg/1' % WEB_PAGE_BASE
...
URI_PAGE_PATTERN_SEARCH = r'&pg=(\d+)'
RESULTS_PER_PAGE = 79
...
def init():
    ...
    items.append(provider.getMenuItem('Estrenos', 'get_movies', PREMIERE_URI_BASE))
...
def search(uri):
    ...
    items = provider.getItemsFromPattern(uri, patterns, URI_PAGE_PATTERN_SEARCH, False, 'get_torrent_search')
...
def get_torrent(path):        
    ...
    if response:
        ...
        link = provider.getSanitizedLink(path, link)
